...
def main():
    ...
    if args.do_train:
        ...
        example_segment_ids = torch.tensor([])
        example_input_mask = torch.tensor([])
...
#                iteration_start = time.time()
                ...
                example_segment_ids = segment_ids
                example_input_mask = input_mask
                ...
                if (step + 1) % args.gradient_accumulation_steps == 0:
                    ...
                    iters_per_epoch = int(len(train_examples) / args.train_batch_size / args.gradient_accumulation_steps) + 1
                    workload_data['ELAPSED_ITERATIONS'] = str(global_step)
                    workload_data['TOTAL_ITERATIONS'] = str(int(num_train_optimization_steps))
                    workload_data['ITERATIONS_PER_EPOCH'] = str(iters_per_epoch)
                    workload_data['TOTAL_EPOCHS'] = str(int(args.num_train_epochs)) 
...
#                    hydra_estimated_epoch_duration = elapsed_duration_since_start + (iters_per_epoch - elapsed_iteration) * current_iteration_duration
...
#                prev_iteration_duration = current_iteration_duration
        ...
        summary(model, input_data=[example_input_ids, example_segment_ids, example_input_mask])

        flops, params = profile(model, (example_input_ids, example_segment_ids, example_input_mask))
        ...
        torch.onnx.export(model, (example_input_ids, example_segment_ids, example_input_mask), onnx_model_path, verbose=False)
