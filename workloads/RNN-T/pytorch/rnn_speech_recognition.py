...
def main():
    ...
    total_batch_size = batch_size * world_size * args.grad_accumulation_steps
    ...
    example_input_feats = torch.tensor([])
    example_input_feat_len = torch.tensor([])
    example_input_txt = torch.tensor([])
    example_input_txt_len = torch.tensor([])
    ...
    for epoch in range(start_epoch + 1, args.epochs + 1):
        ...
        for batch in train_loader:
            ...
            if not flag:
                example_input_feats = feats
                example_input_feat_len = feat_lens
                example_input_txt = txt
                example_input_txt_len = txt_lens
                flag = True
    ...
    summary(model, input_data=[example_input_feats, example_input_feat_len, example_input_txt, example_input_txt_len])

    flops, params = profile(model, (example_input_feats, example_input_feat_len, example_input_txt, example_input_txt_len))
    ...
    torch.onnx.export(model, (example_input_feats, example_input_feat_len, example_input_txt, example_input_txt_len), onnx_model_path, verbose=False, opset_version=11)
