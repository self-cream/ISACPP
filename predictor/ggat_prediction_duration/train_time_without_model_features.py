...
from dataset import InterferenceWithoutModelFeaturesDataset, collate
...
interference_dataset = InterferenceWithoutModelFeaturesDataset(dataset_dir_path)
...
model = GGAT_with_3_blocks(in_dim=10, hidden_dim=512, out_dim=1, dropout=1, num_heads=3).to(device)
...
for epoch in range(3000):
    ...
    torch.save(model.state_dict(), './ablation_trained_models/ours_without_model_features/{}.pkl'.format(epoch))
    ...
    files = glob.glob('./ablation_trained_models/ours_without_model_features/*.pkl')
    for file in files:
...
torch.save(model.state_dict(), './ablation_trained_models/ours_without_model_features/the-last.pkl')

print('Loading {}th epoch'.format(best_epoch))
model.load_state_dict(torch.load('./ablation_trained_models/ours_without_model_features/{}.pkl'.format(best_epoch)))
