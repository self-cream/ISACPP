...
model_without_gpu_features = GGAT_with_3_blocks(in_dim=19, hidden_dim=512, out_dim=1, dropout=1, num_heads=3).to(device)
model_without_model_features = GGAT_with_3_blocks(in_dim=10, hidden_dim=512, out_dim=1, dropout=1, num_heads=3).to(device)
model_without_hyperparameter_features = GGAT_with_3_blocks(in_dim=19, hidden_dim=512, out_dim=1, dropout=1, num_heads=3).to(device)
...
model.load_state_dict(torch.load('./trained_models/6/261.pkl'))
...
model_without_gpu_features.load_state_dict(torch.load('./ablation_trained_models/ours_without_gpu_features/161.pkl'))
...
for test_graph in test_dataloader_without_gpu_features:
    ...
    denormalized_preds = preds.reshape(-1) * (
                colocation_data_max['execution_time'] - colocation_data_min['execution_time']) + colocation_data_min[
                             'execution_time']
    denormalized_truth = test_graph.ndata['execution_time_label'] * (
                colocation_data_max['execution_time'] - colocation_data_min['execution_time']) + colocation_data_min[
                             'execution_time']
...
model_without_model_features.load_state_dict(torch.load('./ablation_trained_models/ours_without_model_features/650.pkl'))
...
model_without_hyperparameter_features.load_state_dict(torch.load('./ablation_trained_models/ours_without_hyperparameter_features/209.pkl'))
