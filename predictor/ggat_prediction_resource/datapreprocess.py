...
def get_max_and_min_in_colocation_data():
    ...
    allocated_memory_values = workload_colocation_data_remove_nil['GPU memory (MB)'].str.split(' / ').apply(
        lambda x: [float(i) for i in x])
    memory_util_values = workload_colocation_data_remove_nil['PCIe bandwidth'].str.split(' / ').apply(
        lambda x: [float(i) for i in x])
    ...
    allocated_memory_values_list = allocated_memory_values.tolist()
    min_allocated_memory_value = min(min(sublist) for sublist in allocated_memory_values_list)
    max_allocated_memory_value = max(max(sublist) for sublist in allocated_memory_values_list)
    colocation_data_min['gpu_memory'] = min_allocated_memory_value
    colocation_data_max['gpu_memory'] = max_allocated_memory_value

    pcie_band_values_list = memory_util_values.tolist()
...
if __name__ == '__main__':
    ...
    load_normalized_csv_and_process_data_for_graph(rtx_4090_colocation_csv_file, numeric_columns, solo_data_max,
