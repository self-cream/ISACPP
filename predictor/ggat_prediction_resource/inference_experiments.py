...
resource_util_model = GGAT_with_3_blocks(in_dim=24, hidden_dim=512, out_dim=3, dropout=1, num_heads=3).to(device)

resource_util_model.load_state_dict(torch.load('./trained_models/6/24.pkl'))
resource_util_model.eval()
...
def get_inference_results(solo_csv_file_path, colocation_csv_file_path, workload_list, workload_batch_size_list, colocation_workload_name, colocation_workload_batch_size_list):
    ...
    for idx in range(len(workload_list)):
        ...
        with torch.no_grad():
            normalized_pred_resource = resource_util_model(current_graph, current_graph.ndata['node_features'])
        ...
        inference_results_list.append(pred_resource)

    return inference_results_list


if __name__ == '__main__':
    inference_results_list = get_inference_results(rtx_4090_solo_csv_file, rtx_4090_colocation_csv_file, rtx_4090_workloads, rtx_4090_workload_batch_sizes, rtx_4090_colocation_workloads, rtx_4090_colocation_workload_batch_sizes)
